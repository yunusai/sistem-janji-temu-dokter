            ...
            <div class="p-4 bg-white shadow-sm sm:p-8 sm:rounded-lg">
                <div class="max-w-xl">
                    <section>
                        ...
                        <form class="mt-6" id="formPeriksa" action="{{ route('dokter.memeriksa.update', $periksa->id) }}" method="POST">
                            @csrf
                            @method('PATCH')
                            ...
                            <div class="mb-3 form-group">
                                ...
                                <input
                                    ...
                                    id="namaPasien"
                                    name="nama_pasien"
                                    value="{{ $periksa->janjiPeriksa->pasien->nama }}"
                                >
                            </div>
                                ...
                                <select class="rounded form-control" id="obat" name="obat[]" multiple>
                                    @foreach ($obats as $obat)
                                        <option value="{{ $obat->id }}" data-harga="{{ $obat->harga }}"
                                            {{ (isset($detailPeriksa) && $detailPeriksa->contains('id_obat', $obat->id)) ? 'selected' : '' }}>
                            ...
                            {{-- id janji periksa --}}
                            <input type="hidden" name="id_janji_periksa" value="{{ $periksa->id }}">
